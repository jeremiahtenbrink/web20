...
class GetStart extends React.Component{
    ...
    render(){
        ...
        return ( <Container fluid>
                <Row className="cover">
                    <Col xs="6" className="text-center align-self-center">
                        { this.props.newUser ? ( <div className="form">
                                <h3>We need more...</h3>
                                <p className="text-danger">
                                    Use same name as on Airtable including
                                    casing
                                </p>
                                <Input
                                    type="text"
                                ...
                                <br/>
                                <Button onClick={ this.submitHandler }>Get
                                    Started</Button>
                            </div> ) : ( <>
                                <h1>Welcome PM</h1>
                                <h6>Login or Signup</h6>
                                <Button
                                    onClick={ this.props.signIn }>Google</Button>
                            </> ) }
                    </Col>
                    <Col xs="6" className="cover-img"/>
                </Row>
...
const mapStateToProps = ( { auth } ) => ( {
    isLoading: auth.isLoading, newUser: auth.newUser, user: auth.user
} );

export default connect( mapStateToProps, { signIn, createUser } )( GetStart );
