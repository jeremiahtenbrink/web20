...
import {
    editStudent, generateStudentLink, getStudentLessons
} from "../actions";
...
class Student extends Component{
    ...
    componentWillUpdate( nextProps, nextState, nextContext ){
        if( nextProps.students && nextProps.students.length > 0 &&
            !nextState.loaded && nextState.studentId ){
            const student = nextProps.students.filter( student => {
                return student.id === nextState.studentId;
            } );
            const { firstName, lastName, github } = student[ 0 ];
            this.setState( { firstName, lastName, github, loaded: true } );
            this.props.getStudentLessons( nextState.studentId, this.props.uid );
        }
    }
                ...
                <Col md={ 6 } className="text-center align-self-center">
                    { this.state.loaded ? ( <>
                        ...
                        <Form onSubmit={ this.onSubmit }>
                            ...
                            ( <h6>
                                Link: { this.props.students[ this.state.studentId ].link }
                            </h6> ) }
...
const mpts = state => ( {
    ...
    uid: state.auth.uid,
    lessons: state.students.studentLessons,
} );
