...
class AttendanceReport extends Component {
  ...
  getReportLink = () => {
    let url =
      "https://airtable.com/shr6wexWV3RM4ITJP?prefill_Project+Manager=Jeremiah%20Tenbrink%20(WEB20)";

    if (this.state.studentSubmission !== "") {
      url += `&prefill_Student+Submission=${encodeURI(
        this.state.studentSubmission
      )}%20(Introduction to User Interface and Git)`;
    }
    ...
    if (this.state.sprintRating !== "") {
      url += `&prefill_Sprint+Challenge+Rating=${encodeURI(
        this.state.sprintRating
      )}`;
    }
    ...
    return (
      <div className={"attendance-report"}>
        ...
        <Form>
          <Input
            type={"select"}
            value={this.state.studentSubmission}
          ...
          <Input
            type={"text"}
            value={this.state.threeWordsByPm}
        ...
        <a className={"attendance-link"} href={this.getReportLink()}>
          {this.getReportLink()}
        </a>
      </div>
    );
