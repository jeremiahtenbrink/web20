...
class AttendanceReport extends Component {
  ...
  getReportLink = () => {
    let url =
      "https://airtable.com/shripCmauVlvxNrAT?prefill_Project+Manager=Jeremiah%20Tenbrink%20(WEB20)&prefill_Sections=WEB20";
    ...
    if (this.state.notHere.length > 0) {
      ...
      for (let j = 0; j < this.state.notHere.length; j++) {
        ...
        url += `${this.state.notHere[j].firstName}%20${
          this.state.notHere[j].lastName
        }`;
      }
    }
